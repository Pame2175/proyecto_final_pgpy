...
function calcularPorciones(cantPersonas) {
    const alimentoSeleccionado = document.getElementById("alimento").value;

    const gramosPorPersona = propiedadesAlimenticias[alimentoSeleccionado].gramosPorPersona;
    const mlPorPorcion = propiedadesAlimenticias[alimentoSeleccionado].mlPorPorcion;
    const total = gramosPorPersona * cantPersonas;
    const mlTotal = mlPorPorcion * cantPersonas;

    let mensaje = `El total para ${cantPersonas} personas es: ${total} gramos y la cantidad de agua para preparar es: ${mlTotal} ml (${calcularTazasDeAgua(mlTotal, cantPersonas)} tazas).`;

    if (alimentoSeleccionado === 0) {
        const tazasArroz = calcularTazasDeArroz(gramosPorPersona);
        mensaje += ` Para el arroz, necesitarás aproximadamente ${tazasArroz} tazas.`;
        // Agregar información sobre tazas por porción
        mensaje += ` ${calcularTazasDeAgua(mlPorPorcion, 1)}`;
    } else if (alimentoSeleccionado === 1) {
        // No hay taza específica para los frijoles, puedes agregar lógica según tus preferencias.
    } else if (alimentoSeleccionado === 2) {
        const tazasFideos = calcularTazasDeFideos(gramosPorPersona);
        mensaje += ` Para los fideos, necesitarás aproximadamente ${tazasFideos} tazas.`;
        // Agregar información sobre tazas por porción
        mensaje += ` ${calcularTazasDeAgua(mlPorPorcion, 1)}`;
    }
...
// Nueva función para calcular tazas de agua
function calcularTazasDeAgua(mlDeAgua) {
    // Definir la capacidad estándar de una taza (puedes ajustar este valor según sea necesario)
    const capacidadTazaMl = 240;
    // Calcular la cantidad de tazas
    const tazas = mlDeAgua / capacidadTazaMl;
    return tazas.toFixed(2);
}
